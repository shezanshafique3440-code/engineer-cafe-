...
1. Render → **New → Blueprint**, pick this repository and branch.
2. Render prompts for the values marked `sync: false` — set `SEED_ADMIN_EMAIL`,
...
3. Apply. The build runs `prisma migrate deploy`, seeds the menu **only while
...
4. Sign in at `/admin/login` with the admin credentials you entered.

Two caveats on Render's free tier: the free database expires after 30 days, and
a free web service sleeps after inactivity so the first request is slow. Set
